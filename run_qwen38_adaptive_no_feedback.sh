...
"$UV_BIN" run --with-requirements requirements-cosmos.txt \
  python preflight_qwen38_adaptive_rf20.py \
  --dataset-root "$DATASET_ROOT" \
  --max-examples-per-class 10 \
  --report "$RUN_ROOT/preflight.json"

if [[ "$PREFLIGHT_ONLY" == "1" ]]; then
  echo "Adaptive RF20 preflight completed; API calls skipped."
  exit 0
fi
...
"$UV_BIN" run --with-requirements requirements-cosmos.txt \
  python validate_qwen38_adaptive_smoke.py \
  --run-root "$SMOKE_ROOT" | tee "$SMOKE_ROOT/smoke_validation.json"

if [[ "$SMOKE_ONLY" == "1" ]]; then
  echo "Adaptive live smoke test passed; full inference skipped."
  exit 0
fi
...
for dataset in "${datasets[@]}"; do
  ...
  for ((attempt = 1; attempt <= MAX_PASSES; attempt++)); do
    ...
    echo "$dataset adaptive resumable pass $attempt/$MAX_PASSES exited $status." >&2
    sleep 10
  done
  if [[ $status -ne 0 ]]; then
...
"$UV_BIN" run --with-requirements requirements-cosmos.txt \
  python aggregate_qwen38_adaptive_rf20.py \
  --dataset-root "$DATASET_ROOT" \
  --run-root "$RUN_ROOT"

echo "Complete adaptive no-feedback RF20-VL-FSOD benchmark succeeded."
