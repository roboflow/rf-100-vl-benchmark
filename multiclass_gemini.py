...
def create_few_shot_prompt(train_folder, categories_dict):
    ...
    if not os.path.exists(annotation_file):
        raise ValueError(f"Few-shot annotation file not found: {annotation_file}")
    ...
    for img_id in annotated_image_ids:
        if img_id not in images_by_id:
            raise ValueError(f"Image ID {img_id} found in annotations but not in image list. Skipping.")
        ...
        if not os.path.exists(image_path):
            raise ValueError(f"Few-shot image file not found: {image_path}. Skipping example.")
...
def precompute_prompts_for_dataset(dataset_dir, categories, categories_dict):
    ...
    basic_system_prompt = "Return bounding boxes as a JSON array with labels. Never return masks or code fencing." #"Return bounding boxes as a JSON array with labels in the format [{'label': 'class_name', 'box_2d': [ymin, xmin, ymax, xmax]}]. The coordinates should be normalized from 0 to 1000. Never return masks or code fencing around the final JSON array itself."
    final_query_text_basic = f"Detect the 2d bounding boxes of the following objects: {category_prompt}."

    instructions_system_prompt = basic_system_prompt #f"Return bounding boxes as a JSON array with labels. Never return masks or code fencing. Use the attached {dataset_name} dataset annotator instructions as a reference for better predictions."
    if instructions:
        ...
        raise ValueError(f"No instructions found for dataset {dataset_dir}")
    
    few_shot_contents_list = create_few_shot_prompt(train_folder, categories_dict) 
    few_shot_system_prompt = basic_system_prompt #f"Return bounding boxes as a JSON array with labels. Never return masks or code fencing. Use the attached detection examples as a reference."
    final_query_text_few_shot = final_query_text_basic

    combined_system_prompt = basic_system_prompt #f"Return bounding boxes as a JSON array with labels. Never return masks or code fencing. Use the attached {dataset_name} dataset annotator instructions and the detection examples as a reference."
...
def process_image(args):
    ...
    try:
        ...
        if few_shot_content_list_orig:
            ...
            if mode == 'combined' and instructions_text:
                if contents and contents[0].role == "user":
                    ...
                    if text_part_modified or (not text_part_modified and new_first_parts) :
                        ...
                        raise ValueError(f"Failed to modify/prepend instructions in the first user turn for image {image_id}.")
                else:
                    raise ValueError(f"Unexpected content structure for first user turn in 'combined' mode for image {image_id}.")
        ...
        if system_prompt is None or not contents:
             raise ValueError("Failed to construct valid system_prompt or contents for API call.")
...
def process_dataset(dataset_dir, few_shot, just_instructions, combined, model_id, output_dir_root, visualize_dir_root):
    ...
    if (few_shot or combined) and not os.path.exists(train_folder):
        raise FileNotFoundError(f"Train folder not found in {dataset_dir}")
    ...
    if os.path.exists(train_folder):
        try:
            ...
            raise Exception(f"Failed to visualize few-shot ground truth for {dataset_name}: {vis_err}")
    else:
        raise Exception(f"Skipping few-shot ground truth visualization for {dataset_name} as train folder is missing.")

    prompts = precompute_prompts_for_dataset(dataset_dir, category_names, categories_dict)

    images = annotations.get("images", [])
    if not images:
        raise ValueError(f"No test images found in annotation file for dataset {dataset_name}.")
...
def main():
    ...
    parser.add_argument('--dataset_location', type=str, default="rf100_datasets",
    ...
    parser.add_argument('--model_id', type=str, default= "gemini-2.5-pro-preview-03-25", #"gemini-2.5-flash-preview-04-17", #"gemini-2.0-flash"
