...
import os
from shared_eval_utils import is_rate_limit_error
...
def visualize_few_shot_ground_truth(train_folder, categories_dict, output_vis_dir, logger):
    ...
    if not os.path.exists(annotation_file):
        raise Exception(f"Few-shot annotation file not found for visualization: {annotation_file}")
    ...
    for img_id in annotated_image_ids:
        ...
        if img_id not in images_by_id:
            raise Exception(f"Image ID {img_id} found in annotations but not in image list.")
        ...
        if not os.path.exists(image_path):
            raise Exception(f"Few-shot image file not found: {image_path}. Skipping visualization.")

        example_annotations = annotations_by_image[img_id]
        if not example_annotations:
             raise Exception(f"No annotations found for image ID {img_id} unexpectedly. Skipping.")
...
def inference_with_retry(contents, system_prompt, model_id, logger, rate_limiter, client):
    ...
    while retries <= MAX_RETRIES:
        try:
            ...
            #print the text of the first content object
            #print(f"First content object text: {contents[0].parts[0].text}")
            #print(f"Second content object text: {contents[1].parts[0].text}")
            #print(f"Third content object text: {contents[2].parts[0].text}")
            #print(f"Fourth content object text: {contents[3].parts[0].text}")
            ...
            if response_text is None:
                 ...
                 try:
                      if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                           ...
                           if response_text is None:
                                raise ValueError("First part has no text.")
                           logger.info("Recovered response text from candidate parts.")
                      else:
                           raise ValueError("No candidates or parts found.")
                 except Exception as e:
...
def visualize_predictions(image_path, boxes, input_width, input_height, 
    ...
    if frame is None:
        raise Exception(f"Could not read image at {image_path}. Skipping visualization.")
