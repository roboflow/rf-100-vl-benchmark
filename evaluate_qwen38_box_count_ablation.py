...
def select_reference_sequences(
    ...
    *,
    ...
    """Select nested, diverse, train-only boxes with one source image per shot.
    ...
    pixels. The ordering never uses test images or model predictions.
    """
    ...
    for category_id, category_name in categories.items():
        # Keep one representative box per image. RF20's K-shot definition is
        # instance based, but distinct source images make the evidence-count
        # intervention interpretable as cross-image prompting.
        ...
        for annotation in annotations_by_category[category_id]:
            ...
            if (area, -int(annotation["id"])) > (
                ...
                best_by_image[image_id] = annotation
        if len(best_by_image) < required_count:
            raise ValueError(
                f"{category_name!r} has {len(best_by_image)} distinct positive "
                f"train images; {required_count} are required."
            )

        candidates: list[dict[str, Any]] = []
        for image_id, annotation in best_by_image.items():
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    references = select_reference_sequences(train, train_directory)
