...
def inference_with_retry_og(messages, model_id=MODEL_ID):
    """
    ...
    while retries <= MAX_RETRIES:
        try:
            ...
            #print(f"Sending messages to {model_id}: {messages}")
...
def create_few_shot_messages_qwen(train_folder, categories_dict):
    ...
    if not os.path.exists(annotation_file):
        raise Exception("Few-shot annotation file not found")

    try:
        with open(annotation_file, 'r') as f:
            train_data = json.load(f)
    except Exception as e:
        raise Exception("Failed to load or parse few-shot annotations")
    ...
    for img_id in annotated_image_ids:
        if img_id not in images_by_id:
            raise Exception(f"Image ID {img_id} found in annotations but not in image list. Skipping.")
        ...
        if not os.path.exists(image_path):
            raise Exception(f"Few-shot image file not found: {image_path}. Skipping example.")

        base64_image_example = encode_image(image_path)
        if not base64_image_example:
            raise Exception(f"Failed to encode few-shot image: {image_path}")
        ...
        assistant_response_text = format_ground_truth_for_model_qwen(
            example_annotations, categories_dict, width, height
        )
...
def precompute_prompts_for_dataset_qwen(dataset_dir, categories, categories_dict):
    ...
    if os.path.exists(readme_path):
        ...
        raise Exception(f"README.dataset.txt not found in {dataset_dir}. Instructions will be empty.")

    basic_system_prompt = SYSTEM_PROMPT

    category_prompt = ", ".join(categories)
    final_query_text_basic = f"Locate all of the following objects: {category_prompt} (each of those is a separate class) in the image and output the coordinates in JSON format."

    instructions_system_prompt = basic_system_prompt
    if instructions:
         final_query_text_instructions_standalone = f"Locate all of the following objects: {category_prompt} (each of those is a separate class) in the image and output the coordinates in JSON format.\n\nUse the following annotator instructions to improve detection accuracy:\n{instructions}\n"
    else:
    ...
    if os.path.exists(train_folder):
        ...
        if not few_shot_messages_list:
             raise Exception(f"Train folder {train_folder} exists but failed to create few-shot examples.")
    else:
        raise Exception(f"Train folder {train_folder} not found. Cannot create few-shot examples.")
...
def convert_to_coco_format_og(qwen_predictions, image_id, original_width, original_height, 
...
def process_image(args):
    ...
    if status_dict.get(image_key) is True:
         ...
         # OLD and INCORRECT for the new flow:
         # return None, None
         # NEW and CORRECT:
    ...
    try:
        ...
        if not base64_image_test:
             raise ValueError(f"Failed to encode test image: {image_path}")
            ...
            if mode == 'combined' and instructions_text:
                if few_shot_messages and few_shot_messages[0].get("role") == "user":
                    ...
                    if not text_part_modified:
                        raise ValueError(f"Could not find 'text' part in first user message's content to prepend instructions for image {image_id}. Instructions may be missing from history.")
                elif few_shot_messages:
                    raise ValueError(f"Expected first few-shot message to be 'user' but found '{few_shot_messages[0].get('role')}' for image {image_id} in combined mode. Cannot prepend instructions.")

            processed_few_shot_messages = few_shot_messages

        elif (few_shot or combined):
             raise Exception(f"Mode '{mode}' selected for image {image_id} but no few-shot examples were available. Proceeding without them.")
        ...
        response_text, err = inference_with_retry_og(
        ...
        if err:
            ...
            coco_annotations, original_boxes_for_vis = convert_to_coco_format_og(
                response_text, image_id, original_width, original_height,
                input_width, input_height, categories_dict
            )
...
def process_dataset(dataset_dir, few_shot, just_instructions, combined, model_id, output_dir_root, visualize_dir_root):
    ...
    if (few_shot or combined) and not os.path.exists(train_folder):
         raise Exception(f"Train folder {train_folder} not found for dataset {dataset_name}. Few-shot/Combined modes will lack examples.")
    ...
    if not images:
        raise Exception(f"No test images found in annotation file for dataset {dataset_name}. Skipping test processing.")
...
def main():
    ...
    parser.add_argument('--model_id', type=str, default="qwen2.5-vl-72b-instruct", # Make model configurable
    ...
    if not all_dataset_dirs:
        raise Exception(f"No datasets found in {args.dataset_dir}. Ensure it contains subdirectories with 'test' folders.")
    ...
    for dataset_dir in tqdm(all_dataset_dirs, desc="Processing datasets", unit="dataset"):
        ...
        try:
            ...
            logger.info(f"Finished processing dataset: {dataset_name}")
             ...
             raise Exception(f"Skipping dataset {dataset_name} due to FileNotFoundError: {fnf_err}")
        except Exception as e:
            raise Exception(f"CRITICAL error processing dataset {dataset_name}: {e}")
